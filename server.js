...
async function initDB(){
  try {
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS profils (
      ...
      preuve_referent VARCHAR(120) DEFAULT NULL, preuve_hash VARCHAR(80) DEFAULT NULL,
      badge ENUM('bronze','argent','or') DEFAULT 'bronze',
      vitrine_premium TINYINT(1) DEFAULT 0, vitrine_expire DATETIME DEFAULT NULL,
      signale TINYINT(1) DEFAULT 0, consentement TINYINT(1) DEFAULT 0,
      cree_le TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;`);

    await pool.query(`CREATE TABLE IF NOT EXISTS avis (
      id INT AUTO_INCREMENT PRIMARY KEY, profil_id INT NOT NULL,
      employeur_id VARCHAR(60) NOT NULL, mission_id INT DEFAULT NULL,
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS employeurs (
      ...
      commune VARCHAR(120) DEFAULT NULL, contact VARCHAR(60) DEFAULT NULL,
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS missions (
      ...
      employeur_type VARCHAR(40), profil_id INT DEFAULT NULL,
      titre VARCHAR(150) NOT NULL, description TEXT,
      commune VARCHAR(120), budget VARCHAR(60),
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS candidatures (
      ...
      statut2 ENUM('Postulee','Acceptee','Refusee','EnCours','Terminee','ConfirmeeJeune','Confirmee','Avisee','Annulee') DEFAULT 'Postulee',
      motif_annulation VARCHAR(200) DEFAULT NULL,
      date_acceptee DATETIME DEFAULT NULL,
      date_demarree DATETIME DEFAULT NULL,
      date_terminee DATETIME DEFAULT NULL,
      date_confirmee DATETIME DEFAULT NULL,
      date_avisee DATETIME DEFAULT NULL,
      cree_le TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;`);
    ...
    await pool.query(`CREATE TABLE IF NOT EXISTS signalements (
      ...
      cible_type ENUM('profil','avis','photo','mission') NOT NULL,
...
app.put('/api/profils/:id', auth, async (req,res) => {
  try {
    ...
    await pool.query('UPDATE profils SET nom=?,metier=?,commune=?,description=?,contact=? WHERE id=?',
...
app.put('/api/profils/:id/preuve', auth, async (req,res) => {
  try {
    ...
    await pool.query(
      'UPDATE profils SET preuve_url=?,preuve_type=?,preuve_lien=?,preuve_referent=?,preuve_hash=? WHERE id=?',
      [preuve||null, preuve_type||null, preuve_lien||null, preuve_referent||null, h, req.params.id]
    );
    res.json({ ok:true });
  } catch(e){ res.status(500).json({ error:e.message }); }
});
...
app.put('/api/profils/:id/premium', auth, async (req,res) => {
  try {
    ...
    res.json({ ok:true, vitrine_premium:1, expire:exp });
  } catch(e){ res.status(500).json({ error:e.message }); }
});
...
app.post('/api/employeurs/verifier', auth, async (req,res) => {
  try {
    ...
    const { reference, commune, contact } = req.body;
    ...
    await pool.query(
      `INSERT INTO employeurs (id,categorie,reference,verifie,en_attente,commune,contact) VALUES (?,?,?,?,?,?,?)
       ON DUPLICATE KEY UPDATE categorie=VALUES(categorie),reference=VALUES(reference),
         verifie=GREATEST(verifie,VALUES(verifie)),en_attente=VALUES(en_attente),
         commune=VALUES(commune),contact=VALUES(contact)`,
      [empId, cat, reference, isVerified, enAttente, commune||null, contact||null]
    );
...
app.post('/api/missions/:id/postuler', auth, async (req,res) => {
  try {
    ...
    await pool.query(
      'INSERT INTO candidatures (mission_id,profil_id,employeur_id,statut2) VALUES (?,?,?,"Postulee")',
    ...
    if (emp.length) await notifier(emp[0].id, 'candidature', 'Nouvelle candidature', `Quelqu'un a postulé à "${m[0].titre}".`, '/employeur/candidatures');
  ...
  try {
    ...
    const [r] = await pool.query(
      `SELECT c.*, m.titre AS mission_titre, m.commune AS mission_commune,
       ...
       WHERE c.employeur_id=? ORDER BY c.cree_le DESC LIMIT 100`,
      [empId]
    );
    res.json(r);
  } catch(e){ res.status(500).json({ error:e.message }); }
  ...
  try {
    ...
    const [r] = await pool.query(
      `SELECT c.*, m.titre AS mission_titre, m.commune AS mission_commune, m.budget AS mission_budget,
       ...
       FROM candidatures c
       LEFT JOIN missions m ON m.id=c.mission_id
       WHERE c.profil_id=? ORDER BY c.cree_le DESC LIMIT 100`,
...
app.post('/api/candidatures/:id/accepter', auth, async (req,res) => {
  try {
    ...
    if (c[0].employeur_id !== req.compte.employeur_id && c[0].employeur_id !== ('emp-'+req.compte.id))
      return res.status(403).json({ error:'Non autorisé' });
...
app.post('/api/candidatures/:id/demarrer', auth, async (req,res) => {
  try {
    ...
    const [pj] = await pool.query('SELECT id FROM comptes WHERE profil_id=?', [c[0].profil_id]);
    if (pj.length) await notifier(pj[0].id, 'candidature', 'Mission démarrée', 'La mission est en cours.', '/jeune/profil');
    res.json({ ok:true });
  } catch(e){ res.status(500).json({ error:e.message }); }
});
...
app.post('/api/candidatures/:id/confirmer', auth, async (req,res) => {
  try {
    ...
    if (emp.length) await notifier(emp[0].id, 'candidature', 'Mission confirmée', 'Le jeune a confirmé. Tu peux laisser un avis.', '/employeur/candidatures');
    res.json({ ok:true });
  } catch(e){ res.status(500).json({ error:e.message }); }
});
...
app.post('/api/candidatures/:id/annuler', auth, async (req,res) => {
  try {
    ...
    const [c] = await pool.query('SELECT * FROM candidatures WHERE id=?', [req.params.id]);
    if (!c.length) return res.status(404).json({ error:'Introuvable' });
    const isPartie = (req.compte.type === 'jeune' && c[0].profil_id === req.compte.profil_id)
...
app.get('/api/recherches', auth, async (req,res) => {
  const [r] = await pool.query('SELECT * FROM recherches_sauvegardees WHERE compte_id=? ORDER BY cree_le DESC LIMIT 30', [req.compte.id]);
  res.json(r);
});

app.post('/api/recherches', auth, async (req,res) => {
  const { metier, commune } = req.body;
  await pool.query('INSERT INTO recherches_sauvegardees (compte_id,metier,commune) VALUES (?,?,?)', [req.compte.id, metier||null, commune||null]);
  res.json({ ok:true });
});
...
app.post('/api/signalements', auth, async (req,res) => {
  try {
    ...
    await pool.query('INSERT INTO signalements (cible_type,cible_id,motif,signale_par) VALUES (?,?,?,?)',
      [cible_type, cible_id, cleanComment(motif), req.compte.id]);
    res.json({ ok:true });
  } catch(e){ res.status(500).json({ error:e.message }); }
});
